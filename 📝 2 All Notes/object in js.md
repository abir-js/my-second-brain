...
- The **value** is the data (it can be _any_ data type: a number, a string, a boolean, an array, or even another object).
...
**A. Creating an Object** The most common and best way to create an object is using the **object literal** syntax: curly braces `{}`.
...
**B. Accessing (Reading) Properties** There are two ways to get a value from an object.
...
**C. Updating and Adding (Creating) Properties** You use the same syntax for both. If the property exists, it's updated. If it doesn't, it's created.
...
**D. Deleting Properties** Use the `delete` keyword.
...
**A. The `for...in` Loop (The Old Way)** This loop iterates over the **keys** (property names) of an object.
...
_(**Advanced Gotcha:** `for...in` also iterates over properties from the object's prototype chain, which can be unexpected. For this reason, the modern methods below are usually preferred.)_

**B. The Modern `Object` Methods (The Better Way)** These methods return arrays, which you can then loop over with a `for...of` loop.
...
**A. Objects are a Reference Type** This is the same critical concept we saw with arrays. When you assign an object to another variable, you only copy the **reference** (the memory address). Both variables point to the **same object**.
...
**B. How to Copy an Object (Shallow Copy)** To create a true copy of an object's top-level properties, use the **spread syntax (`...`)** or `Object.assign()`.
...
_(**Note:** This is a "shallow" copy. If the object contains other objects, those nested objects will still be references, not copies.)_
...
- **Computed Property Names:** Use a variable as a property key _during creation_ by putting it in brackets.
...
**First Thought: "For each property key _in_ this object, do this."**
...
|**`for...in`**|**Generally avoid.** Use only if you specifically _need_ to inspect inherited properties.|Can cause bugs by iterating over the prototype chain. The other methods are safer.|
...
**The Modern Way: Spread Syntax (`...`) (ES2018)** This is the most common, readable, and preferred way to make a shallow copy.
...
**The Old Way: `Object.assign()`** This does the same thing as the spread syntax. `const shallowCopyAssign = Object.assign({}, originalUser);`
...
**A. Creating a Symbol** You create a symbol by calling the `Symbol()` factory function. Every symbol is unique.
...
**B. Using a Symbol as an Object Key** To use a symbol as a key, you **must use bracket notation `[]`**.
...
**C. Symbols are "Hidden" from Normal Iteration** Symbol properties are not included in `for...in` loops or `Object.keys()`, which makes them ideal for metadata that shouldn't interfere with normal object operations.
...
---
![[Pasted image 20251019130829.png]]